...
const ProjectGenPostsTab: React.FC<ProjectGenPostsProp> = ({ }) => {
    ...
    const generateMockPosts = (count: number, baseId: number = 0): GenPostContent[] => {
    ...
    const handleSubmit = (e: React.FormEvent<HTMLFormElement>) => {
        ...
        setTimeout(() => {
            setPostContent(prevPosts => {
                const maxId = prevPosts.length > 0 ? Math.max(...prevPosts.map(p => p.id)) : 0;
    ...
    useEffect(() => {
        if (editor) {
            const post = selectedPostIndex ? postContent[selectedPostIndex] : postContent[0];
            if (post) {
        ...
        >
            ...
            <div className="flex flex-col pl-4 border-mountain-200 border-r-1 w-[25%] h-full">
                <div className="flex justify-between items-end pr-2 pb-2 border-mountain-200 border-b-1 w-full h-18">
                    ...
                    <div onClick={() => { setGenerateMode(true), setSelectedPostIndex(null) }} className="flex items-center space-x-2 bg-mountain-50 hover:bg-mountain-50/60 shadow-sm p-2 px-4 rounded-full text-sm cursor-pointer">
                        <Plus />
                        <p>New Post</p>
                    </div>
                </div>
                <div className="flex flex-col space-y-2 p-2">
                    {postContent.length > 0 ? postContent.map((_, index) => (
                        <div
                            key={index}
                            onClick={() => { setSelectedPostIndex(index), setGenerateMode(false) }}
                        ...
                        >
                            <p className="w-[70%] text-mountain-600 text-sm line-clamp-1">Post {index + 1}</p>
                        ...
                        <p className="text-mountain-600 text-sm">This project currently has no post.</p>
                    )}
                </div>
            </div>
            ...
            <div className="flex flex-col space-y-2 bg-mountain-50 w-[75%] h-full overflow-y-auto">
                        ...
                        <div className="relative flex items-end gap-4 pl-4 w-xl">
                            <div className="flex flex-col">
                                <label className="flex items-center space-x-2 mb-1 text-mountain-800 text-sm">
                                    ...
                                    <span><Info className="size-4" /></span>
                                </label>
                                <Input className="rounded-md w-108 h-10 placeholder:text-mountain-400" placeholder="Generate your post content" />
                            </div>
                            <div className="flex flex-col">
                                ...
                                <PopoverContent
                                    ...
                                    <div className="space-y-4">
                                        ...
                                        <div className="space-y-2">
                                            <label className="px-1 text-mountain-800 text-sm">Tone of Voice</label>
                                        ...
                                        <div className="flex flex-col gap-2">
                                            <div className="flex items-center space-x-2">
                                                <label className="text-mountain-800 text-sm">Number of words</label>
                                                <Input
                                                    className="w-16 h-8"
                                                    type="number"
                                                    value={charCount}
                                        ...
                                        <div className="flex items-center space-x-2">
                                            <label className="text-mountain-800 text-sm">Generate hashtag</label>
                                            <Switch
                                        ...
                                        <div className="flex items-center space-x-2">
                                            <label className="text-mountain-800 text-sm">Include emojis</label>
                                            <Switch
                                                checked={useEmojis}
                ...
                {(postContent && postContent.length > 0 && !generateMode) ? (
                                ...
                                <div className="flex items-center gap-2 bg-white px-2 border-mountain-200 border-b rounded-t-md h-12 shrink-0">
                                    {editor && (
                                        <div className="flex justify-between items-center w-full">
                                            ...
                                            {editor && (
                                                <div className={`select-none flex hover:opacity-100 hover:z-50 transform ease-in-out duration-300 bg-white opacity-50 rounded-md p-2 text-xs text-mountain-600 character-count ${editor.storage.characterCount.words() === MAX_WORDS ? 'character-count--warning' : ''}`}>
                    ...
                    <div className="relative flex flex-col justify-center items-center gap-4 ml-4 w-xl h-[520px]">
                        ...
                        <p className="text-mountain-400 text-sm">Prompt for your post content to automate posting workflow</p>
                    </div>
                )}
            </div>
        </form >
