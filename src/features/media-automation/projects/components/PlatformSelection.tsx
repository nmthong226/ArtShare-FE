...
const PlatformSelection = ({ isEditMode = false }: PlatformSelectionProps) => {
  ...
  return (
    <div className="flex flex-col">
      <div className="flex flex-col justify-between items-center space-x-4 space-y-4 w-xl h-full">
        <div className="flex justify-center items-center bg-indigo-50 pl-2 rounded-full w-fit">
          <Typography variant="h6" component="h1" className='mr-2 font-normal'>
          ...
          <Select disabled={isEditMode} onValueChange={handlePlatformTypeChange} value={selectedPlatform?.name || platformTypeToFetch || ''}>
            <SelectTrigger className="bg-white rounded-full w-40 data-[size=default]:h-10 font-medium text-lg cursor-pointer">
            ...
            <SelectContent className="border-mountain-100 w-full">
              {allAvailablePlatformTypes.map((type) => (
                <SelectItem key={type} value={type} className='text-lg'>
                  {type === 'FACEBOOK' ? <img src={'/public/fb_icon.png'} alt="Facebook" className="inline-block w-4 h-4" /> : type === 'INSTAGRAM' ? <img src={'/public/ins_icon.png'} className="inline-block w-4 h-4" /> : null}
          ...
          {!isLoading && !error && platformTypeToFetch === 'FACEBOOK' && (
            <>
              {fetchedPlatforms.length > 0 ? (
                ...
                <div className="flex flex-col items-center p-6 border-2 border-gray-300 border-dashed rounded-lg text-center">
