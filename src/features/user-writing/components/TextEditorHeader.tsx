...
const TextEditorHeader: React.FC<TextEditorHeader> =
    ({
        ...
        const dynamicWidth = Math.min(baseWidth + Math.max(0, (documentTitle.length - 36) * 8), maxWidth);
            ...
            <nav className={`top-0 z-50 sticky px-4 flex justify-between items-center bg-gradient-to-r from-indigo-100 via-purple-50 to-pink-50 dark:bg-mountain-950 border-b-1 border-b-mountain-100 dark:border-b-mountain-700 w-full h-16`}>
                ...
                <div className='top-1/2 left-1/2 absolute flex justify-between items-center space-x-2 w-[720px] -translate-x-1/2 -translate-y-1/2'>
                    ...
                    <div className='flex space-x-2'>
                        <Button type='submit' onClick={handleSaveDocument} disabled={isSaving} className='flex justify-center items-center bg-white hover:bg-mountain-50 border border-mountain-200 rounded-full w-32 h-12 text-mountain-950 cursor-pointer'>
                            {isSaving ? (
                                <>
                ...
                <Dialog open={confirmOpen} onClose={handleCloseConfirm}>
                    ...
                    <DialogContent className='flex flex-col space-y-2'>
                        <p className='text-mountain-600'>This will make your document publicly available in the blog section for others to read. Ready to publish?</p>
                    ...
                    <DialogActions className='flex justify-between px-6'>
                        ...
                        <div className='flex space-x-2'>
                            <Button onClick={handleCloseConfirm} className='bg-mountain-200 hover:bg-mountain-200/80 text-mountain-950'>Cancel</Button>
                            <Button
