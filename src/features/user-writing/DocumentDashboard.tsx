...
import { CircularProgress, DialogContentText, Input, ToggleButton, ToggleButtonGroup, Tooltip } from "@mui/material";
...
const DocumentDashboard = () => {
    ...
    const { mutate: updateTitle, isPending: isUpdating } = useMutation({
        mutationFn: ({ docId, newTitle }: { docId: number; newTitle: string }) => updateDocName(docId, newTitle),
        onSuccess: () => {
            queryClient.invalidateQueries({ queryKey: ["myBlogs"] });
    ...
    return (
        <div className="flex flex-col items-center h-screen overflow-auto sidebar">
                    ...
                    <div className="flex items-center">
                        <div className="flex">
                            <ToggleButtonGroup
                                ...
                                aria-label="Platform"
                            >
                                <ToggleButton value="doc" className="px-4 rounded-l-full h-10 font-normal text-base capitalize">Document</ToggleButton>
                                <ToggleButton value="blog" className="px-4 rounded-r-full h-10 font-normal text-base capitalize">Blog</ToggleButton>
                            </ToggleButtonGroup>
                        </div>
                        <div className="flex">
                    ...
                    <div className="items-start gap-6 grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 p-6 pb-96 min-h-[calc(100vh-4rem)]">
                        {myBlogs.map((blog, index) => (
                            <div key={index} className="flex flex-col justify-center items-center space-y-4 bg-white pb-2 border border-mountain-200 hover:border-indigo-600 rounded-lg">
                                <div onClick={() => handleNavigateToEdit(blog.id)} className="flex justify-center items-end bg-mountain-50 border border-mountain-50 rounded-t-lg w-full aspect-square overflow-hidden">
                                    ...
                                    <div className="flex justify-between items-center w-full">
                                        ...
                                        <Popover>
                                            ...
                                            <PopoverContent className="mt-2 mr-20 p-0 border-mountain-200 w-48 font-thin text-sm">
                                                <div onClick={() => handleOpenChangeTitleDialog(blog)} className="flex items-center space-x-2 hover:bg-mountain-50 px-3 py-2 cursor-pointer">
                                                    <FiEdit />
                                                    <p>Rename</p>
                                                </div>
                                                <hr className="flex border-mountain-200 border-t-1 w-full" />
                                                <div onClick={() => { setConfirmDialogOpen(true); setSelectedBlogId(blog.id); }} className="flex items-center space-x-2 hover:bg-mountain-50 px-3 py-2 cursor-pointer">
                                                    <IoTrashBinOutline />
                                                    <p>Delete this {blog.is_published ? 'blog' : 'document'}</p>
                                                </div>
                                            </PopoverContent>
                                        </Popover>
                                    </div>
            ...
            <Dialog open={openChangeTitleDialog} onClose={handleCloseChangeTitleDialog}>
                ...
                <DialogActions>
                    <Button onClick={handleCloseChangeTitleDialog} className='bg-mountain-200 hover:bg-mountain-200/80 text-mountain-950'>Cancel</Button>
                    <Button
            ...
            <Dialog open={confirmDialogOpen} onClose={() => setConfirmDialogOpen(!confirmDialogOpen)}>
                <DialogTitle className='flex items-center space-x-2'>
                    <IoTrashBinOutline />
                    <p>Delete This Document</p>
                </DialogTitle>
                <DialogContent className="space-y-4 w-108">
                    <DialogContentText>Delete this document permanently. Are you sure?</DialogContentText>
                </DialogContent>
                <DialogActions>
                    <Button onClick={() => setConfirmDialogOpen(false)} className='bg-mountain-200 hover:bg-mountain-200/80 text-mountain-950'>Cancel</Button>
                    <Button
                        onClick={() => {
